...
def extract_features(window):
    ...
    for sensor_axis in range(window.shape[1]):
        ...
        features.append(
            np.nonzero(np.diff(sensor_data > 0))[0].size / sensor_data.shape[0])  # Taxa de Cruzamento de Zero
...
def load_data_only_one(root_dir):

    test_files_names = [name for name in listdir(root_dir + '/sensor_data/') if 'mw_ew20' in name]

    test_data = load_and_extract(root_dir + '/sensor_data/', files_names=test_files_names, w_overlap=0)
    return test_data
